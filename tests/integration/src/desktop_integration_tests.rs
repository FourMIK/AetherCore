...
use aethercore_identity::{IdentityManager, PlatformIdentity, Attestation};
...
fn create_signed_telemetry(
    ...
    let message = format!("{}:{}:{}", node_id, serde_json::to_string(&data).unwrap(), timestamp);
...
async fn test_desktop_identity_registration_and_lookup() {
    ...
    let identity = PlatformIdentity {
        ...
        attestation: Attestation::Software { certificate: vec![] },
    ...
    
    {
        ...
        identity_mgr.register(identity).expect("Failed to register identity");
    }
    ...
    // Verify identity can be retrieved
    {
        ...
        assert!(retrieved.is_some(), "Identity should be retrievable after registration");
        
        let retrieved = retrieved.unwrap();
        assert_eq!(retrieved.id, node_id);
        assert_eq!(retrieved.public_key.len(), 32, "Ed25519 public key should be 32 bytes");
    }
}
...
async fn test_desktop_telemetry_signature_verification_valid() {
    ...
    let (signature_b64, timestamp, _message) = create_signed_telemetry(&node_id, telemetry_data.clone(), &signing_key);
    ...
    let message = format!("{}:{}:{}", node_id, serde_json::to_string(&telemetry_data).unwrap(), timestamp);
    let message_hash = blake3::hash(message.as_bytes());
...
async fn test_desktop_telemetry_signature_verification_invalid() {
    ...
    let (signature_b64, timestamp, _) = create_signed_telemetry(&node_id, telemetry_data.clone(), &wrong_signing_key);
    ...
    let result = verifying_key.verify(message_hash.as_bytes(), &signature);
    assert!(result.is_err(), "Invalid signature should fail verification - FAIL-VISIBLE SECURITY");
}
...
async fn test_desktop_telemetry_unknown_node_rejection() {
    ...
    assert!(identity.is_none(), "Unknown node should not be found - FAIL-VISIBLE SECURITY");
}
...
async fn test_desktop_stream_integrity_tracking() {
    ...
    // Verify stream exists in tracker
    {
        ...
        assert!(status.is_some(), "Stream should exist in tracker after creation");
        
        let status = status.unwrap();
        assert_eq!(status.stream_id, stream_id);
        assert!(!status.is_compromised, "New stream should not be compromised");
...
async fn test_desktop_stream_integrity_merkle_vine_validation() {
    ...
    // Verify stream integrity
    {
        ...
        assert!(!status.is_compromised, "Valid Merkle chain should not be compromised");
...
async fn test_desktop_stream_integrity_broken_chain_detection() {
    ...
    // Verify broken chain is detected
    {
        ...
        assert!(status.is_compromised, "Broken Merkle chain should be detected - FAIL-VISIBLE SECURITY");
        assert_eq!(status.total_events, 2, "Should have 2 events");
        assert_eq!(status.broken_events, 1, "One event should be broken");
        assert!(status.compromise_reason.is_some(), "Should have compromise reason");
    }
}
...
async fn test_desktop_genesis_bundle_generation() {
    ...
    assert_eq!(decoded_pub.len(), 32, "Ed25519 public key should be 32 bytes");
    assert_eq!(decoded_sig.len(), 64, "Ed25519 signature should be 64 bytes");
    
    // Verify signature
    let verifying_key_check = VerifyingKey::from_bytes(&decoded_pub.as_slice().try_into().unwrap()).unwrap();
    ...
    assert!(verify_result.is_ok(), "Genesis bundle signature should verify");
}
...
async fn test_desktop_ffi_error_handling_invalid_node_id() {
    ...
    assert!(!is_valid, "Empty node_id should be rejected with descriptive error");
}
...
async fn test_desktop_ffi_error_handling_invalid_signature_format() {
    ...
    assert!(result.is_err(), "Malformed base64 should be rejected with descriptive error");
}
...
async fn test_desktop_audit_event_generation_signature_verification_failure() {
    ...
    assert!(audit_event["reason"].as_str().unwrap().contains("spoofing"), 
