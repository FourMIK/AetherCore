...
use aethercore_core::{
    Event, EventBuilder, EventCategory, EventLedger, EventSeverity,
    MerkleAggregator, MerkleTree,
    SlashingEngine, NodeState, ByzantineFaultType,
...
async fn test_end_to_end_chain_of_trust() {
    ...
    ledger.append_signed_event(signed_event1).expect("Failed to append event 1");
    ledger.append_signed_event(signed_event2).expect("Failed to append event 2");
    ...
    use aethercore_core::{AggregationConfig, preprocess_leaves};
    ...
    let event_tuples = ledger.iterate_events(1, 10).expect("Failed to fetch events");
    ...
    assert_eq!(node_state, NodeState::Unknown, "Node should start as Unknown");
    ...
    assert_eq!(final_state, NodeState::Healthy, "Node should remain healthy for valid events");
    ...
    trust_chain.add_genesis(genesis_link).expect("Failed to add genesis link");
    ...
    trust_chain.add_link(trust_link).expect("Failed to add trust link");
...
async fn test_slashing_engine_detects_byzantine_fault() {
    ...
    let final_state = slashing_engine.get_node_state(&node_id);
    assert_eq!(final_state, NodeState::Revoked, "Node should be revoked after equivocation");
