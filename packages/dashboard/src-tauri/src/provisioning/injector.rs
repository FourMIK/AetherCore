...
pub async fn inject_pi(
    ...
    let tcp = TcpStream::connect(format!("{}:22", ip))
        .map_err(|e| {
            if e.kind() == std::io::ErrorKind::ConnectionRefused {
                format!("FAIL-VISIBLE: Connection refused to {}. \
                         Ensure SSH is enabled and device is on network.", ip)
            } else if e.kind() == std::io::ErrorKind::TimedOut {
                format!("FAIL-VISIBLE: Connection timeout to {}. \
                         Check network connectivity and firewall.", ip)
            } else {
    ...
    let mut sess = Session::new()
        .map_err(|e| format!("FAIL-VISIBLE: Failed to create SSH session: {}", e))?;
    ...
    let payload_path = find_payload_binary()
        .map_err(|e| format!("FAIL-VISIBLE: {}", e))?;
    ...
    for line in output.lines() {
        ...
        if let Ok(json_value) = serde_json::from_str::<serde_json::Value>(trimmed) {
                ...
                || json_value.get("node_id").is_some()
            {
                ...
                let node_id = json_value.get("node_id")
                    .or_else(|| json_value.get("root"))
                    .and_then(|v| v.as_str())
                    .unwrap_or("unknown")
                    .to_string();
                
                let public_key = json_value.get("pub_key")
                    .or_else(|| json_value.get("public_key"))
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
                    .to_string();
                
                let root_hash = json_value.get("root")
