...
impl FlirBridgeConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            ...
            username: std::env::var("FLIR_USERNAME")
                .unwrap_or_else(|_| "admin".to_string()),
            password: std::env::var("FLIR_PASSWORD")
...
pub async fn start_flir_bridge(
    ...
    edge_node_ip: String,
    udp_port: u16,
) -> Result<FlirBridge, Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_config_from_env() {
        let config = FlirBridgeConfig::from_env();
        assert!(!config.flir_ip.is_empty());
