...
export class SigningServiceClient {
  ...
  constructor(config: SigningServiceConfig) {
    ...
    if (this.config.useTls) {
      ...
      try {
        ...
        if (this.config.clientKeyPath && this.config.clientCertPath) {
          ...
          console.log('[SigningServiceClient] Using mTLS credentials');
        } else {
          // Server-only TLS
          credentials = grpc.credentials.createSsl(rootCert);
          console.log('[SigningServiceClient] Using TLS credentials');
        }
      } catch (error) {
        throw new Error(
      ...
      console.warn(
        '[SigningServiceClient] WARNING: Using insecure connection. This is NOT suitable for production.',
      );
    }
    ...
    console.log(
      `[SigningServiceClient] Connected to ${this.config.serverAddress} (TLS: ${this.config.useTls ? 'enabled' : 'disabled'})`,
    );
  }
