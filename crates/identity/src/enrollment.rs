...
impl EnrollmentContext {
    ...
    pub fn generate_identity(&mut self, platform_id: &str) -> crate::Result<PlatformIdentity> {
        tracing::info!(
            "Generating TPM-backed identity for platform: {}",
            platform_id
        );
        ...
        let ak = self
            .tpm
            .generate_attestation_key(format!("{}-ak", platform_id))?;
        ...
        Ok(PlatformIdentity {
            ...
            attestation: Attestation::Tpm {
                quote: serde_json::to_vec(&quote).map_err(|e| {
                    crate::Error::Identity(format!("Failed to serialize quote: {}", e))
                })?,
                pcrs: serde_json::to_vec(&quote.pcrs).map_err(|e| {
                    crate::Error::Identity(format!("Failed to serialize PCRs: {}", e))
                })?,
...
mod tests {
    ...
    fn test_sign_challenge() {
        ...
        assert!(verifying_key
            .verify(b"tampered-challenge", &parsed_signature)
            .is_err());
