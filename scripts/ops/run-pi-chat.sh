...
REMOTE_BIN="${REMOTE_BIN:-/usr/local/bin/coderalphie-chat}"
...
exec ssh -t "${SSH_OPTS[@]}" "$PI_SSH" \
  "REMOTE_USER='${REMOTE_USER}' REMOTE_BIN='${REMOTE_BIN}' bash -lc '
set -euo pipefail
...
if [[ -n \"\${c2_ws_url}\" ]]; then
  echo \"Launching chat with C2_WS_URL=\${c2_ws_url}\"
  exec sudo -u \"\${REMOTE_USER}\" env C2_WS_URL=\"\${c2_ws_url}\" \"\${REMOTE_BIN}\"
fi

echo \"WARN: C2_WS_URL not found from coderalphie service. Falling back to chat defaults.\"
exec sudo -u \"\${REMOTE_USER}\" \"\${REMOTE_BIN}\"
'"
